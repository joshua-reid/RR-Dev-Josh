...
Reply.ValidateZipCode = function (geoData) {
  var url = 'http://m.reply.com/Handler/SubmitAutoClickHandler.ashx?',
    ...
    params = 'zipcode=' + zip_code + '&trimid=' + current_model_id + '&adid=' + Reply.adid;

  // append subid to params when it exists
  if ( Reply.subid !== "" && Reply.subid !== null ) params = params + '&subid=' + Reply.subid;
...
Reply.SetWaitQueueId = function (wqidData) {
  if (wqidData.success === true) {
    ...
    Reply.CallLeadSubmit();
  } else {
    Reply.waitQueueId = 'rejected';
  }
};

Reply.CallLeadSubmit = function () {
  // formerly known as $('#ContactSubmit').click, now this function is called from Reply.SetWaitQueueId
  var url_lead = 'http://m.reply.com/Handler/SubmitAutoLeadHandler.ashx?',
    contact_form = $('fieldset#contact_info'),
    current_model_id = $('#TopPcModelDropDown option:selected').val(),
  ...
  Reply.GetAjaxData(Reply.SubmittedLead, url_lead + params);
};
...
jQuery(document).ready(function () {
  ...
  if (jQuery("#WidgetAutos")) {
    ...
    jQuery('#ContactSubmit').click(function (event) {
      ...
      var url_zip = 'http://m.reply.com/Handler/CityStateZipValidationHandler.ashx',
        contact_form = $('fieldset#contact_info'),
      ...
      if (Validation.validateAll( contact_form )) {
        ...
        contact_form.find('input[type=submit]').val('loading...').css({opacity: '0.5', filter: 'alpha(opacity=50)'}).attr('disabled', 'disabled');

        // validate zipcode
        if (current_model_id !== "" && current_model_id !== null && zip_code.length === 5) {
          Reply.GetAjaxData(Reply.ValidateZipCode, url_zip + '?input=' + zip_code);
        } else {
          $("#first_name").select();
        }
      }
    ...
    // $('#ContactSubmit').click was here, look for Reply.CallLeadSubmit
    ...
    jQuery('a.sem-btn-automotive').click(function (event) {
      ...
      // focus zip
      setTimeout( function () {$('#first_name').select();}, 600 );
      // custom event
